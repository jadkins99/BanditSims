...
class Agent(object):
    ...
    def observe(self, reward,action_attempt = None):
        
        if action_attempt is None:
            action_attempt = self.last_action
        
        self.action_attempts[action_attempt] += 1

        if self.gamma is None:
            g = 1 / self.action_attempts[action_attempt]
        else:
            g = self.gamma
        q = self._value_estimates[action_attempt]

        self._value_estimates[action_attempt] += g*(reward - q)
        self.t += 1
...
class BetaAgent(Agent):
    ...
    The Beta Agent is a Bayesian approach to a bandit problem with a Bernoulli
     ...
     a conjugate prior.
    """
    def __init__(self, bandit, policy, ts=True,ic = False):
        ...
        # self.model = pm.Model()
        # with self.model:
        self.prior = pm.Beta.dist(alpha=np.ones(self.k),
    ...
    def observe(self, reward,action_attempt = None):
        ...
        if self.ic and action_attempt is not None:
            self.action_attempts[action_attempt] += 1
            self.alphas[action_attempt] += reward
            self.betas[action_attempt] += self.n - reward
        else:
            self.action_attempts[self.last_action] += 1
            self.alphas[self.last_action] += reward
            self.betas[self.last_action] += self.n - reward
            
        self.prior = pm.Beta.dist(alpha=self.alphas,
                                  beta=self.betas, shape=(1, self.k),
                                  transform=None)
